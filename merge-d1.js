...
console.log('========================================');
console.log('多池隔离系统 - D1 版本合并工具');
console.log('========================================\n');

// 定义 D1 版本的模块文件顺序（按依赖关系排序）
const modules = [
  'constants.js',
  'utils.js',
  'd1-storage.js',        // 只使用 D1 存储
  'model-fetcher.js',
  'pool-manager.js',
  'routing.js',
  'streaming.js',
...
// 输出文件路径
const outputFile = path.join(__dirname, 'worker_d1.js');

console.log('准备合并 D1 版本模块:');
...
// 文件头部
let output = `// ==================== 多池隔离系统 v3.0 (D1 数据库版本) ====================
  ...
  try {
    if (fs.existsSync(modulePath)) {
      ...
      if (module === 'pool-manager.js') {
        // 将 storage.js 的导入替换为直接使用 d1-storage.js 的函数
...
try {
  ...
  console.log('   cp wrangler-d1.toml.example wrangler.toml');
  console.log('');
  console.log('2. 创建 D1 数据库:');
  console.log('   wrangler d1 create gemini-pool-db');
  console.log('');
  console.log('3. 修改 wrangler.toml 中的 database_id，然后部署:');
  console.log('   wrangler secret put ADMIN_PASSWORD');
  console.log('   wrangler deploy');
  console.log('');
  console.log('注意: 数据库表会在首次运行时自动创建');
  console.log('');
